...
static struct early_suspend    s6e8aa5x01_early_suspend;
#endif

static int s6e8aa5x01_get_brightness(struct backlight_device *bd)
{
...
static int s6e8aa5x01_set_brightness(struct backlight_device *bd)
{
...
static const struct backlight_ops s6e8aa5x01_backlight_ops = {
	.get_brightness = s6e8aa5x01_get_brightness,
	.update_status = s6e8aa5x01_set_brightness,
};
...
static int s6e8aa5x01_probe(struct dsim_device *dsim)
{
	...
	bd = backlight_device_register("pwm-backlight.0", NULL,
		NULL, &s6e8aa5x01_backlight_ops, NULL);
	if (IS_ERR(bd))
...
static int s6e8aa5x01_suspend(struct dsim_device *dsim)
{
	return 1;
}

static int s6e8aa5x01_resume(struct dsim_device *dsim)
{
	return 1;
}
...
struct mipi_dsim_lcd_driver s6e8aa5x01_mipi_lcd_driver = {
	...
	.suspend	= s6e8aa5x01_suspend,
	.resume		= s6e8aa5x01_resume,
};
