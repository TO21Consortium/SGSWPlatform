...
//close by qhuo for temporarily passing building
...
#ifdef CONFIG_DTS_WIFI_BCM43438
	{ .compatible = "broadcom,bcm43438_wifi" },
#else
...
static int espresso_wlan_probe(struct platform_device *pdev)
{
	...
	if (device_create_file(&pdev->dev, &dev_attr_wifi_test))
		pr_info("[WIFI] bcm43438 wif_test sys file create failed\n");
	return ret;
}
