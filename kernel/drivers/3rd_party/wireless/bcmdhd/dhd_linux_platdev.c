...
#endif
#if defined(CONFIG_SAMSUNG_WIFI_CONTROL)
...
#ifdef CONFIG_SAMSUNG_WIFI_CONTROL
	adapter->wifi_plat_data = &espresso_wifi_control;
#else 
...
static int dhd_wifi_platform_load_pcie(void)
{
	...
	if (dhd_wifi_platdata == NULL) {
		...
		if (true/*dhd_download_fw_on_driverload*/) {
