...
#ifdef QOS_LOCK_AFFINITY
void exynos_march_hotplug_disable(void);
void exynos_march_hotplug_enable(void);
static bool wifi_hotplug_disable = false;
bool task_affinity = false;
static int task_affinity_apply(bool enable)
{
	if (enable) {
		if (wifi_hotplug_disable == false) {
			exynos_march_hotplug_disable();
			wifi_hotplug_disable = true;
		}
		task_affinity = true;
	} else {
		if (wifi_hotplug_disable == true) {
			exynos_march_hotplug_enable();
			wifi_hotplug_disable = false;
		}
		task_affinity = false;
	}
	return 0;
...
static int irq_affinity_apply(bool enable)
{
	if (enable) {
		if (wifi_hotplug_disable == false) {
			exynos_march_hotplug_disable();
		...
		irq_set_affinity(237, cpumask_of(6));
	} else {
		if (wifi_hotplug_disable == true) {
			exynos_march_hotplug_enable();
			wifi_hotplug_disable = false;
...
static int hmpboost_apply(bool enable)
{
	static bool hmpboost_enable = false;
...
static unsigned int qos_wlan[][14]={
	...
	{125*1024*1024/8, 1500000, 0, 1500000, 0, 1552000, 560000,0,0,4,4,0,0,0},
...
static void qos_lock(int level)
{
	unsigned cl1_min_freq, cl1_max_freq, cl0_min_freq, cl0_max_freq, mif_freq, int_freq,
		cl1_min_num, cl1_max_num, cl0_min_num, cl0_max_num;
#ifdef QOS_LOCK_AFFINITY
	unsigned task_affinity_en,irq_affinity_en,hmp_boost_en;
#endif
...
#ifdef QOS_LOCK_AFFINITY
	task_affinity_apply(task_affinity_en);
	irq_affinity_apply(irq_affinity_en);
	hmpboost_apply(hmp_boost_en);	
#endif
}
...
bool dhd_bus_watchdog(dhd_pub_t *dhd)
{
	...
	old_level = current_level;
       if  (tx_rate > qos_wlan[0][0]) {
            ...
            current_level = 1;
        } else if  (tx_rate > qos_wlan[2][0]) {
            current_level = 2;
        } else if  (tx_rate > qos_wlan[3][0] || rx_rate > qos_wlan[3][0]) {
            current_level = 3;
        } else if  (tx_rate > qos_wlan[4][0] || rx_rate > qos_wlan[4][0]) {
            current_level = 4;
        } else {
            current_level = 100;
        }

	if(current_level < 4 && false == status_bl1 && false == lcd_is_on){
		wifi_cl1_get();
		status_bl1 = true;
	}else if(current_level >= 4 && true == status_bl1){
		wifi_cl1_release();
		status_bl1 = false;
	}
...
extern int check_wifi_op(int ch_num);
