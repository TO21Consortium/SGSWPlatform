	...
	u32 gpio_scenario)
{
	...
	/* do configs */
	for (idx = 0; idx < idx_max; ++idx) {
		ret = exynos_fimc_is_module_soc_pin_control(client, pinctrl, &pin_ctrls[scenario][gpio_scenario][idx]);
		if (ret) {
	...
	u32 gpio_scenario)
{
	...
	/* do configs */
	for (idx = 0; idx < idx_max; ++idx) {
		ret = exynos_fimc_is_module_soc_pin_debug(client, pinctrl, &pin_ctrls[scenario][gpio_scenario][idx]);
		if (ret) {
			pr_err("[@] exynos_fimc_is_sensor_gpio(%d) is fail(%d)", idx, ret);
			goto p_err;
		}
