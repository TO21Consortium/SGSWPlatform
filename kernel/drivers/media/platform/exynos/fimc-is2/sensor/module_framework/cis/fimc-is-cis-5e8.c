...
#include "fimc-is-cis-5e8-setA.h"
...
static const u32 *sensor_5e8_global;
static u32 sensor_5e8_global_size;
static const u32 **sensor_5e8_setfiles;
static const u32 *sensor_5e8_setfile_sizes;
static const struct sensor_pll_info **sensor_5e8_pllinfos;
static u32 sensor_5e8_max_setfile_num;

static void sensor_5e8_cis_data_calculation(const struct sensor_pll_info *pll_info, cis_shared_data *cis_data)
{
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_5E8_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_5E8_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_5E8_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_5E8_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

static int sensor_5e8_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
/* CIS OPS */
int sensor_5e8_cis_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
	...
	if (ret < 0) {
		warn("sensor_5e8_check_rev is fail when cis init");
		cis->rev_flag = true;
		ret = 0;
	}

	cis->cis_data->cur_width = SENSOR_5E8_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_5E8_MAX_HEIGHT;
	cis->cis_data->low_expo_start = 33000;
	cis->need_mode_change = false;

	sensor_5e8_cis_data_calculation(sensor_5e8_pllinfos[setfile_index], cis->cis_data);
...
int sensor_5e8_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	...
	sensor_cis_dump_registers(subdev, sensor_5e8_setfiles[0], sensor_5e8_setfile_sizes[0]);
...
static int sensor_5e8_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
...
#else
static inline int sensor_5e8_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	return 0;
}
#endif
 ...
 */
int sensor_5e8_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	ret = sensor_5e8_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_5e8_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	ret = sensor_cis_set_registers(subdev, sensor_5e8_global, sensor_5e8_global_size);
	if (ret < 0) {
...
int sensor_5e8_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	if (mode > sensor_5e8_max_setfile_num) {
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_5e8_check_rev is fail");
			goto p_err;
		}
	}

	sensor_5e8_cis_data_calculation(sensor_5e8_pllinfos[mode], cis->cis_data);

	ret = sensor_cis_set_registers(subdev, sensor_5e8_setfiles[mode], sensor_5e8_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_5e8_set_registers fail!!");
		goto p_err;
	}
...
int sensor_5e8_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_5e8_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_5E8_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_5E8_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_5E8_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_5E8_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_5E8_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_5E8_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_5e8_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	...
	ret = sensor_5e8_cis_group_param_hold_func(subdev, 0x00);
	if (ret < 0)
		err("[%s] sensor_5e8_cis_group_param_hold_func fail\n", __func__);
...
int sensor_5e8_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_5e8_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	hold = sensor_5e8_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
...
p_err:
	if (hold > 0) {
		hold = sensor_5e8_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_5e8_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
...
int sensor_5e8_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_5e8_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_5e8_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_5e8_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_5e8_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_5e8_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_5e8_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_5e8_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_5e8_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_5e8_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_5e8_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_5e8_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_5e8_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_5e8_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
static struct fimc_is_cis_ops cis_ops = {
	.cis_init = sensor_5e8_cis_init,
	.cis_log_status = sensor_5e8_cis_log_status,
	.cis_group_param_hold = sensor_5e8_cis_group_param_hold,
	.cis_set_global_setting = sensor_5e8_cis_set_global_setting,
	.cis_mode_change = sensor_5e8_cis_mode_change,
	.cis_set_size = sensor_5e8_cis_set_size,
	.cis_stream_on = sensor_5e8_cis_stream_on,
	.cis_stream_off = sensor_5e8_cis_stream_off,
	.cis_set_exposure_time = sensor_5e8_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_5e8_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_5e8_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_5e8_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_5e8_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_5e8_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_5e8_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_5e8_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_5e8_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_5e8_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_5e8_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_5e8_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_5e8_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_5e8_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_5e8_cis_get_max_digital_gain,
	.cis_compensate_gain_for_extremely_br = sensor_cis_compensate_gain_for_extremely_br,
	.cis_wait_streamoff = sensor_cis_wait_streamoff,
};

int cis_5e8_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_S5K5E8);
	if (!sensor_peri) {
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_5e8_global = sensor_5e8_setfile_A_Global;
		sensor_5e8_global_size = sizeof(sensor_5e8_setfile_A_Global) / sizeof(sensor_5e8_setfile_A_Global[0]);
		sensor_5e8_setfiles = sensor_5e8_setfiles_A;
		sensor_5e8_setfile_sizes = sensor_5e8_setfile_A_sizes;
		sensor_5e8_pllinfos = sensor_5e8_pllinfos_A;
		sensor_5e8_max_setfile_num = sizeof(sensor_5e8_setfiles_A) / sizeof(sensor_5e8_setfiles_A[0]);
	} else {
...
static int cis_5e8_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_5e8_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-5e8",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_5e8_match);

static const struct i2c_device_id cis_5e8_idt[] = {
	{ SENSOR_NAME, 0 },
};

static struct i2c_driver cis_5e8_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_5e8_match
	},
	.probe	= cis_5e8_probe,
	.remove	= cis_5e8_remove,
	.id_table = cis_5e8_idt
};
module_i2c_driver(cis_5e8_driver);
