...
static struct fimc_is_sensor_cfg config_module_5e8[] = {
	/* 2592x1944@30fps */
	FIMC_IS_SENSOR_CFG(2576, 1934, 30, 19, 0, CSI_DATA_LANES_2),
};


static struct fimc_is_vci vci_module_5e8[] = {
	{
...
static int sensor_module_5e8_power_setpin(struct platform_device *pdev,
...
int sensor_module_5e8_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	fimc_is_sensor_module_parse_dt(pdev, sensor_module_5e8_power_setpin);
	...
	module->pdata = pdata;
	module->dev = dev;
	module->sensor_id = SENSOR_NAME_S5K5E8;
	module->subdev = subdev_module;
	module->device = pdata->id;
	module->client = NULL;
	module->active_width = 2576 + 16;
	module->active_height = 1934 + 10;
	...
	module->bitwidth = 10;
	module->vcis = ARRAY_SIZE(vci_module_5e8);
	module->vci = vci_module_5e8;
	module->sensor_maker = "SLSI";
	module->sensor_name = "S5K5E8";
	module->setfile_name = "setfile_5e8.bin";
	module->cfgs = ARRAY_SIZE(config_module_5e8);
	module->cfg = config_module_5e8;
	module->ops = NULL;
	/* Sensor peri */
...
static int sensor_module_5e8_remove(struct platform_device *pdev)
{
	int ret = 0;
...
static const struct of_device_id exynos_fimc_is_sensor_module_5e8_match[] = {
	{
		.compatible = "samsung,sensor-module-5e8",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_module_5e8_match);

static struct platform_driver sensor_module_5e8_driver = {
	.probe  = sensor_module_5e8_probe,
	.remove = sensor_module_5e8_remove,
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-5E8",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_fimc_is_sensor_module_5e8_match,
	}
};

module_platform_driver(sensor_module_5e8_driver);
