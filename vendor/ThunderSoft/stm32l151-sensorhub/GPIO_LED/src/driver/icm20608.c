...
static unsigned long icm20608_i2c_write(unsigned char slave_addr,unsigned char register_addr,unsigned short register_len,unsigned char *register_value)
{  
  return i2c_master_write_register(ICM20608_I2C_ADDR,register_addr,register_len, register_value);
}

static unsigned long icm20608_i2c_read(unsigned char slave_addr,unsigned char register_addr,unsigned short register_len,unsigned char *register_value)
{
  return i2c_master_read_register(ICM20608_I2C_ADDR,register_addr,register_len, register_value);
}
...
static int set_int_enable(unsigned char enable)
{
    ...
    if (enable)
        ...
        tmp = 0x00;
    if (icm20608_i2c_write(st.hw->addr, st.reg->int_enable, 1, &tmp))
        return -1;
...
{   
    ...
    if (icm20608_i2c_write(st.hw->addr, st.reg->who_am_i, 1, &data)) {
      return -1;
    }
...
int icm_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++) {
        if (ii == st.reg->fifo_r_w || ii == st.reg->mem_r_w)
            continue;
        if (icm20608_i2c_read(st.hw->addr, ii, 1, &data))
            return -1;
    }
    return 0;
}
...
int icm_read_reg(unsigned char reg, unsigned char *data)
{
    ...
    if (reg >= st.hw->num_reg)
        return -1;
    return icm20608_i2c_read(st.hw->addr, reg, 1, data);
}
...
int icm_init(void)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (icm20608_i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    delay_ms(100);
    ...
    if (icm20608_i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data))
        return -1;
...
int icm_lp_accel_mode(unsigned short rate)
{
    ...
    { 
      ...
      if (rate == 1)
          ...
          tmp[0] = INV_LPA_500HZ; 
      if (icm20608_i2c_write(st.hw->addr, st.reg->lp_mode_cfg, 1, tmp))
          goto lp_restore;
      /* Enable LP ACCEL mode, ACCEL_FCHOICE_B=1*/
      if (icm20608_i2c_read(st.hw->addr, st.reg->accel_cfg2, 1, tmp))
          goto lp_restore;

      tmp[0] = BIT_FIFO_SIZE_4096 | BIT_ACCL_FC_B | tmp[0];
      if (icm20608_i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, tmp))
          goto lp_restore;
      
      tmp[0] = BIT_LPA_CYCLE;
      tmp[1] = BIT_STBY_XYZG;
      if (icm20608_i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, tmp))
          goto lp_restore;
...
int icm_lp_6axis_mode(int gyro_avg,unsigned char enable)
{
    ...
    if (icm20608_i2c_read(st.hw->addr, st.reg->lp_mode_cfg, 1, &tmp))
      return -1;  
...
int icm_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    if (icm20608_i2c_read(st.hw->addr, st.reg->raw_gyro, 6, tmp))
        return -1;
...
int icm_get_accel_reg(short *data, unsigned long *timestamp)
{
    ...
    if (icm20608_i2c_read(st.hw->addr, st.reg->raw_accel, 6, tmp))
        return -1;
    data[0] = (tmp[0] << 8) | tmp[1];
    data[1] = (tmp[2] << 8) | tmp[3];
    data[2] = (tmp[4] << 8) | tmp[5];
...
int icm_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (icm20608_i2c_read(st.hw->addr, st.reg->temp, 2, tmp))
        return -1;
    raw = (tmp[0] << 8) | tmp[1];
...
int icm_read_accel_bias(long *accel_bias) {
	unsigned char data[6];
	if (icm20608_i2c_read(st.hw->addr, st.reg->xa_offset, 2, &data[0]))
		return -1;
	if (icm20608_i2c_read(st.hw->addr, st.reg->ya_offset, 2, &data[2]))
		return -1;
	if (icm20608_i2c_read(st.hw->addr, st.reg->za_offset, 2, &data[4]))
		return -1;
...
int icm_read_gyro_bias(long *gyro_bias) {
	unsigned char data[6];
	if (icm20608_i2c_read(st.hw->addr, st.reg->xg_offs_usr, 2, &data[0]))
		return -1;
	if (icm20608_i2c_read(st.hw->addr, st.reg->yg_offs_usr, 2, &data[2]))
		return -1;
	if (icm20608_i2c_read(st.hw->addr, st.reg->zg_offs_usr, 2, &data[4]))
		return -1;
...
int icm_set_gyro_bias_reg(long *gyro_bias)
{
    ...
    if (icm20608_i2c_write(st.hw->addr, st.reg->xg_offs_usr, 2, &data[0]))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->yg_offs_usr, 2, &data[2]))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->zg_offs_usr, 2, &data[4]))
        return -1;
    return 0;
}
...
int icm_set_accel_bias_reg(const long *accel_bias) {
    ...
    if (icm20608_i2c_write(st.hw->addr, st.reg->xa_offset, 2, &data[0]))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->ya_offset, 2, &data[2]))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->za_offset, 2, &data[4]))
        return -1;
...
int icm_reset_fifo(void)
{
    ...
    data = 0;
    if (icm20608_i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->fifo_en, 1, &data))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
        return -1;
    ...
    if (icm20608_i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->fifo_en, 1, &st.chip_cfg.fifo_enable))
        return -1;
...
int icm_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (icm20608_i2c_read(st.hw->addr, st.reg->gyro_cfg, 1, &tmp))
        return -1;
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
    tmp = data | tmp;
    if (icm20608_i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, &tmp))
        return -1;
...
int icm_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 3))
        return 0;
    if (icm20608_i2c_write(st.hw->addr, st.reg->accel_cfg, 1, &data))
        return -1;
...
int icm_set_gyro_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.gyro_lpf == data)
        return 0;
    if (icm20608_i2c_write(st.hw->addr, st.reg->lpf, 1, &data))
        return -1;
...
int icm_get_watermark(unsigned char *threshold) {
  ...
  if(icm20608_i2c_read(st.hw->addr, st.reg->fifo_wm_th, 1, threshold))
     return -1;
...
int icm_set_watermark(unsigned char threshold) {
  ...
  if(icm20608_i2c_write(st.hw->addr, st.reg->fifo_wm_th, 1, &threshold))
     return -1;
...
int icm_set_sample_rate(unsigned short rate)
{
    ...
    data = 1000 / rate - 1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->rate_div, 1, &data))
        return -1;
...
int icm_set_sensors(unsigned char sensors)
{
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        data |= BIT_STBY_XYZA;
    if (icm20608_i2c_write(st.hw->addr, st.reg->pwr_mgmt_2, 1, &data)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
...
int icm_get_int_status(short *status)
{
    ...
    if (!st.chip_cfg.sensors)
        return -1;
    if (icm20608_i2c_read(st.hw->addr, st.reg->dmp_int_status, 2, tmp))
        return -1;
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (st.chip_cfg.fifo_enable & INV_XYZ_ACCEL)
		...
		packet_size = 12;
#endif
    if (icm20608_i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, data))
        return -1;
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (icm20608_i2c_read(st.hw->addr, st.reg->int_status, 1, data))
            return -1;
    ...
    if (icm20608_i2c_read(st.hw->addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
...
int icm_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (icm20608_i2c_read(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        tmp &= ~BIT_AUX_IF_EN;
        if (icm20608_i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &tmp))
            return -1;
        delay_ms(3);
        tmp = BIT_BYPASS_EN;
        ...
        if (icm20608_i2c_write(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
    } else {
...
static int accel_20608_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (icm20608_i2c_read(st.hw->addr, REG_20608_XA_ST_DATA, 3, regs)) {
    	return 0x07;
    }
...
static int gyro_20608_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    if (icm20608_i2c_read(st.hw->addr, REG_20608_XG_ST_DATA, 3, regs)) {
        return 0x07;
    }
...
static int get_st_20608_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    data[0] = 0x01;
    data[1] = 0;
    if (icm20608_i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, data))
        return -1;
    delay_ms(200);
    data[0] = 0;
    if (icm20608_i2c_write(st.hw->addr, st.reg->int_enable, 1, data))
    ...
    if (icm20608_i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    if (icm20608_i2c_write(st.hw->addr, st.reg->user_ctrl, 1, data))
        return -1;
    ...
    if (icm20608_i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, data))
        return -1;
    ...
    while (s < test.packet_thresh) {
		...
		if (icm20608_i2c_read(st.hw->addr, st.reg->fifo_r_w, read_size, data))
						return -1;
		ind = 0;
    ...
    unsigned short lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        if (icm20608_i2c_write(st.hw->addr, st.reg->user_ctrl, 3, data))
            goto lp_int_restore;

        /* Set motion threshold. */
        data[0] = thresh_hw;
        if (icm20608_i2c_write(st.hw->addr, st.reg->motion_thr, 1, data))
            goto lp_int_restore;
        ...
        if (icm20608_i2c_write(st.hw->addr, st.reg->lp_mode_cfg, 1, data))
            goto lp_int_restore;

        /* Enable motion interrupt (ICM20608 version). */
        data[0] = BITS_WOM_EN;
        if (icm20608_i2c_write(st.hw->addr, st.reg->accel_intel, 1, data))
            goto lp_int_restore;
        /* Enable LP ACCEL mode, ACCEL_FCHOICE_B=1*/
        if (icm20608_i2c_read(st.hw->addr, st.reg->accel_cfg2, 1, temp))
        	return -1;
