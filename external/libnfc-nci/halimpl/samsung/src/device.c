...
int device_set_mode(eNFC_DEV_MODE mode)
{
    ...
    OSI_logt("device mode chage: %d -> %d, fd: %d", dev_state, mode, pw_driver);
    ret = ioctl(tr_driver, SEC_NFC_SET_MODE, (int)mode);
    if (!ret)
    {
        ...
        OSI_logt("set mode fail: %d(%s)", errno, strerror(errno));
    }
