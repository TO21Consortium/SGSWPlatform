...
static const std::vector<uint8_t>* CreateTrampoline(EntryPointCallingConvention abi,
  ...
  switch (abi) {
    ...
    // Workaround(sys_sw.sa) :
    // dex2oat generate assembly codes to find address of art_jni_dlsym_lookup_stub() with JNIEnvExt::SelfOffset().
    // But JNIEnvExt::SelfOffset() returns 4 instead of 8 (expectation)
    // since dex2oat was built for a 32bit application.
    case kJniAbi:  // Load via Thread* held in JNIEnv* in first argument (X0).
      __ LoadRawPtr(Arm64ManagedRegister::FromXRegister(IP1),
                      ...
                      Offset(JNIEnvExt::SelfOffset().Int32Value() * 2));
