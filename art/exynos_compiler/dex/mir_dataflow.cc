...
const uint64_t MIRGraph::oat_data_flow_attributes_[kMirOpLast] = {
  ...
  // 126 BRANCH_TO_POST_EXIT
  0,

  // 127 MIR_LOWER_BOUND_CHECK
  0,
#endif
};
...
#ifdef EXYNOS_ART_OPT
    LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->arg[i]], mir);
#endif
  }
}
...
#ifdef EXYNOS_ART_OPT
    LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->vC+i], mir);
#endif
  }
}
...
bool MIRGraph::DoSSAConversion(BasicBlock* bb) {
  ...
  for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
...
#ifdef EXYNOS_ART_OPT
        LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->vA], mir);
#endif
        if (df_attributes & DF_A_WIDE) {
          HandleSSAUse(mir->ssa_rep->uses, d_insn->vA+1, num_uses++);
#ifdef EXYNOS_ART_OPT
          LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->vA+1], mir);
#endif
        }
      }
      if (df_attributes & DF_UB) {
...
#ifdef EXYNOS_ART_OPT
        LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->vB], mir);
#endif
        if (df_attributes & DF_B_WIDE) {
          HandleSSAUse(mir->ssa_rep->uses, d_insn->vB+1, num_uses++);
#ifdef EXYNOS_ART_OPT
          LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->vB+1], mir);
#endif
        }
      }
      if (df_attributes & DF_UC) {
...
#ifdef EXYNOS_ART_OPT
        LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->vC], mir);
#endif
        if (df_attributes & DF_C_WIDE) {
          HandleSSAUse(mir->ssa_rep->uses, d_insn->vC+1, num_uses++);
#ifdef EXYNOS_ART_OPT
          LinkSSADefToUseMIR(vreg_to_ssa_map_[d_insn->vC+1], mir);
#endif
        }
      }
    }
    if (df_attributes & DF_HAS_DEFS) {
      HandleSSADef(mir->ssa_rep->defs, d_insn->vA, 0);
#ifdef EXYNOS_ART_OPT
      LinkSSADefToDefMIR(vreg_to_ssa_map_[d_insn->vA], mir);
#endif
      if (df_attributes & DF_A_WIDE) {
        HandleSSADef(mir->ssa_rep->defs, d_insn->vA+1, 1);
#ifdef EXYNOS_ART_OPT
        LinkSSADefToDefMIR(vreg_to_ssa_map_[d_insn->vA+1], mir);
#endif
      }
    }
  }
...
#ifdef EXYNOS_ART_OPT
void MIRGraph::LinkSSADefToUseMIR(int def_ssa_reg, MIR* use_mir) {
  def_ssa_reg_to_use_mir_list_[def_ssa_reg].push_back(use_mir);
}

void MIRGraph::DumpDefSSARegToUseMIRList() {
  std::map<int, std::vector<MIR*> >::iterator it;
  for (it = def_ssa_reg_to_use_mir_list_.begin(); it != def_ssa_reg_to_use_mir_list_.end(); it++) {
    int def_ssa_reg = it->first;
    std::vector<MIR*> &use_mir_list = it->second;
    if (cu_->exynos_verbose) {
      LOG(INFO) << "---------def_ssa_reg : " << def_ssa_reg << "-----------------";
      LOG(INFO) << "# of use mir : " << use_mir_list.size();
    }
    for (std::vector<MIR*>::iterator vit = use_mir_list.begin(); vit != use_mir_list.end(); vit++) {
      MIR* use_mir = *vit;
      if (use_mir->ssa_rep->num_defs == 0)
        continue;

      int use_mir_def_ssa_reg = use_mir->ssa_rep->defs[0];
      if (cu_->exynos_verbose)
        LOG(INFO) << "use_mir's def_ssa_reg : " << use_mir_def_ssa_reg;
    }
  }
}

void MIRGraph::LinkSSADefToDefMIR(int def_ssa_reg, MIR* def_mir) {
  def_ssa_reg_to_def_mir_[def_ssa_reg] = def_mir;
}

void MIRGraph::DumpDefSSARegToDefMIR() {
  std::map<int, MIR*>::iterator it;
  for (it = def_ssa_reg_to_def_mir_.begin(); it != def_ssa_reg_to_def_mir_.end(); it++) {
    int def_ssa_reg = it->first;
    MIR* def_mir = it->second;
    if (cu_->exynos_verbose)
      LOG(INFO) << "---------def_ssa_reg : " << def_ssa_reg << "-----------------";

    int def_mir_def_ssa_reg = def_mir->ssa_rep->defs[0];
    if (cu_->exynos_verbose)
      LOG(INFO) << "def_mir's def_ssa_reg : " << def_mir_def_ssa_reg;

    DCHECK(def_ssa_reg == def_mir_def_ssa_reg) << "def_ssa_reg is not eualt to def_mir_def_ssa_reg";
  }
}
#endif
