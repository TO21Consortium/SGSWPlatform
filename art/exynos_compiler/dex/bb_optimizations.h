...
class CodeLayout : public PassME {
 public:
  ...
  CodeLayout() : PassME("CodeLayout", kAllNodes, "2_post_layout_cfg") {
#else
...
#ifdef EXYNOS_ART_OPT
/**
 * @class ConstantPropagationExynos
 * @brief Perform a constant propagation pass before induction variable.
 *        remove touching reg_location_ in SetConstant & SetConstantWide
 */
class ConstantPropagationExynos : public PassME {
 public:
  ConstantPropagationExynos() : PassME("ConstantPropagation") {
  }
  ...
  bool Worker(PassDataHolder* data) const {
    ...
    BasicBlock* bb = down_cast<PassMEDataHolder*>(data)->bb;
    DCHECK(bb != nullptr);
    c_unit->mir_graph->DoConstantPropagationExynos(bb);
    // No need of repeating, so just return false.
    return false;
  }
};
