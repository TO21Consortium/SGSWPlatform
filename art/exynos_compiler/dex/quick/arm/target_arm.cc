      ...
      if (nc == '!') {
        strcpy(tbuf, "!");
         ...
         switch (*fmt++) {
           ...
           case 'X':
             snprintf(tbuf, arraysize(tbuf), "q%d", RegStorage::RegNum(operand));
             break;
           case 'Y':
             snprintf(tbuf, arraysize(tbuf), "d%d[0]", RegStorage::RegNum(operand));
             break;
           case 'y':
             snprintf(tbuf, arraysize(tbuf), "d%d[1]", RegStorage::RegNum(operand));
             break;
#endif
           default:
