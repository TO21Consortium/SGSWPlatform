...
#include "dex/exynos/mir_graph_opt.h"
...
#ifdef EXYNOS_ART_OPT
  cu.mir_graph.reset(new MIRGraphOpt(&cu, &cu.arena));
#if EXYNOS_ART_QUICK_COMPILER_ON
  ((MIRGraphOpt *)cu.mir_graph.get())->InitArtDebug(true, PrettyMethod(method_idx, dex_file));
#else
  ((MIRGraphOpt *)cu.mir_graph.get())->InitArtDebug(false, PrettyMethod(method_idx, dex_file));
#endif
#else
...
Mir2Lir* QuickCompiler::GetCodeGenerator(CompilationUnit* cu, void* compilation_unit) {
  ...
  switch (cu->instruction_set) {
    case kThumb2:
#ifdef EXYNOS_ART_OPT
      mir_to_lir = ExynosArmCodeGenerator(cu, cu->mir_graph.get(), &cu->arena);
#else
    ...
    case kArm64:
#ifdef EXYNOS_ART_OPT
      mir_to_lir = ExynosArm64CodeGenerator(cu, cu->mir_graph.get(), &cu->arena);
#else
