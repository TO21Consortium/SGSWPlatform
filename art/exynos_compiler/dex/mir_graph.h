...
class BasicBlock : public DeletableArenaObject<kArenaAllocBB> {
 public:
  ...
  bool deopt_loop_head:1;
  bool deopt_loop_end:1;
  bool loop_pre_head:1;
  bool loop_head:1;
  bool loop_end:1;
  bool loop_exiting:1;
  bool loop_exit:1;
  bool applied_codelayout:1;
  uint16_t sebb_loop_nesting_depth;
#endif
...
#ifdef EXYNOS_ART_OPT
  void DoConstantPropagationExynos(BasicBlock* bb);
  void SetConstantExynos(int32_t ssa_reg, int32_t value);
  void SetConstantWideExynos(int32_t ssa_reg, int64_t value);
#endif
...
#ifdef EXYNOS_ART_OPT
  std::map<int, std::vector<MIR*> > def_ssa_reg_to_use_mir_list_;
  std::map<int, MIR*> def_ssa_reg_to_def_mir_;
  void ResetSSADefToUseMIR() { def_ssa_reg_to_use_mir_list_.clear(); }
  void ResetSSADefToDefMIR() { def_ssa_reg_to_def_mir_.clear(); }
#endif
...
#ifdef EXYNOS_ART_OPT
  void LinkSSADefToUseMIR(int dalvik_reg, MIR* use_mir);
  void LinkSSADefToDefMIR(int ssa_reg, MIR* def_mir);
  void DumpDefSSARegToUseMIRList();
  void DumpDefSSARegToDefMIR();
#endif
