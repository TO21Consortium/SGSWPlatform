...
/* Setup a constant value for opcodes thare have the DF_SETS_CONST attribute */
void MIRGraph::SetConstantExynos(int32_t ssa_reg, int32_t value) {
  is_constant_v_->SetBit(ssa_reg);
  constant_values_[ssa_reg] = value;
}

void MIRGraph::SetConstantWideExynos(int32_t ssa_reg, int64_t value) {
  is_constant_v_->SetBit(ssa_reg);
  is_constant_v_->SetBit(ssa_reg + 1);
  constant_values_[ssa_reg] = Low32Bits(value);
  constant_values_[ssa_reg + 1] = High32Bits(value);
...
void MIRGraph::DoConstantPropagationExynos(BasicBlock* bb) {
  MIR* mir;
    ...
    if (df_attributes & DF_SETS_CONST) {
      if (df_attributes & DF_DA) {
        ...
        switch (d_insn->opcode) {
          ...
          case Instruction::CONST_16:
          case Instruction::CONST:
            SetConstantExynos(mir->ssa_rep->defs[0], vB);
            break;
          case Instruction::CONST_HIGH16:
            SetConstantExynos(mir->ssa_rep->defs[0], vB << 16);
            break;
          case Instruction::CONST_WIDE_16:
          case Instruction::CONST_WIDE_32:
            SetConstantWideExynos(mir->ssa_rep->defs[0], static_cast<int64_t>(vB));
            break;
          case Instruction::CONST_WIDE:
            SetConstantWideExynos(mir->ssa_rep->defs[0], d_insn->vB_wide);
            break;
          case Instruction::CONST_WIDE_HIGH16:
            SetConstantWideExynos(mir->ssa_rep->defs[0], static_cast<int64_t>(vB) << 48);
            break;
          default:
            break;
        }
      ...
      /* Move a register holding a constant to another register */
      if (i == mir->ssa_rep->num_uses) {
        SetConstantExynos(mir->ssa_rep->defs[0], constant_values_[mir->ssa_rep->uses[0]]);
        if (df_attributes & DF_A_WIDE) {
          SetConstantExynos(mir->ssa_rep->defs[1], constant_values_[mir->ssa_rep->uses[1]]);
        }
      }
    }
  }
...
#ifdef EXYNOS_ART_OPT
    bb->deopt_loop_head |= bb_next->deopt_loop_head;
    bb->deopt_loop_end |= bb_next->deopt_loop_end;
    bb->loop_pre_head |= bb_next->loop_pre_head;
    bb->loop_head |= bb_next->loop_head;
    bb->loop_end |= bb_next->loop_end;
    bb->loop_exiting |= bb_next->loop_exiting;
    bb->loop_exit |= bb_next->loop_exit;
    bb->applied_codelayout |= bb_next->applied_codelayout;
#endif
