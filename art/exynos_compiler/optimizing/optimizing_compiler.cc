...
#include "exynos/nodes_opt.h"
#include "exynos/optimizing_compiler_opt.h"
#endif
...
#ifdef EXYNOS_ART_OPT
  RunExynosOptimizations(graph, compiler_driver, compilation_stats_.get(),
                         dex_file, dex_compilation_unit, pass_info_printer, &handles);
  if (!ExynosAllocateRegisters(graph, codegen, pass_info_printer))
    return nullptr;
#else
...
#ifdef EXYNOS_ART_OPT
#if EXYNOS_ART_OPTIMIZING_COMPILER_ON
  HGraph* graph = new (&arena) HGraphOpt(&arena, dex_file, method_idx,
