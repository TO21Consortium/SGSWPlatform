...
#include "exynos/sebb_loop_info.h"
...
class HLoopInformation : public ArenaObject<kArenaAllocMisc> {
 public:
  HLoopInformation(HBasicBlock* header, HGraph* graph)
      : header_(header),
        ...
        loop_inversion_(false),
        old_back_edge_(nullptr),
        loop_defined_variables_(graph->GetArena(), graph->GetCurrentInstructionId(), true),
        loop_invariant_variables_(graph->GetArena(), graph->GetCurrentInstructionId(), true),
        induction_variables_(graph->GetArena(), graph->GetCurrentInstructionId(), true),
        reduction_variables_(graph->GetArena(), graph->GetCurrentInstructionId(), true),
        iv_info_(graph->GetArena(), 1U),
        rv_info_(graph->GetArena(), 1U),
        biv_info_(nullptr),
        is_count_up_loop_(false),
        exiting_blocks_(graph->GetArena(), 1U),
        array_access_info_(graph->GetArena(), 1U),
        exit_cond_(nullptr),
        inclusive_(false),
        loop_exit_iv_(nullptr) {}
#else
        blocks_(graph->GetArena(), graph->GetBlocks().Size(), true) {}
#endif
  ...
  void ClearBlocksBitVector() {
    blocks_.ClearAllBits();
  }

  void SetLoopInversion(bool flag) {
    loop_inversion_ = flag;
  }

  bool IsLoopInversion() {
    return loop_inversion_;
  }

  void SetOldBackEdge(HBasicBlock* block) {
    old_back_edge_ = block;
  }

  bool IsOldBackEdge(const HBasicBlock* block) const {
    if (old_back_edge_ == block)
      return true;

    return false;
  }

  ArenaBitVector* GetLoopDefinedVariables() {
    return &loop_defined_variables_;
  }

  ArenaBitVector* GetLoopInvariantVariables() {
    return &loop_invariant_variables_;
  }

  ArenaBitVector* GetInductionVariables() {
    return &induction_variables_;
  }

  ArenaBitVector* GetReductionVariables() {
    return &reduction_variables_;
  }

  GrowableArray<IVInfo*>& GetIVInfo() {
    return iv_info_;
  }

  GrowableArray<RVInfo*>& GetRVInfo() {
    return rv_info_;
  }

  GrowableArray<ArrayAccessInfo*>& GetArrayAccessInfo() {
    return array_access_info_;
  }

  IVInfo* GetBIVInfo() const {
    return biv_info_;
  }

  void SetBIVInfo(IVInfo* biv_info) {
    biv_info_ = biv_info;
  }
  ...
  bool HasArray() const;
  bool CanApplyExynosBCE();

  void AddExitingBlock(HBasicBlock* block);
  HBasicBlock* GetExitingBlock() const;
  HInstruction* GetExitingConditionSSAValue() const;
  HCondition* GetExitingConditionOpCode() const;
  void SetExitCondition(HInstruction* cond);
  HCondition* GetExitCondition() const;

  /* whether inclusive or not when control goes to the loop body */
  void SetInclusiveForLoopIteration(bool inclusive) {
    inclusive_ = inclusive;
  }

  bool IsInclusiveForLoopIteration() const {
    return inclusive_;
  }

  bool GetConditionForLoopIteration(IfCondition* cond);

  IVInfo* GetLoopExitIVInfo() const {
    return loop_exit_iv_;
  }

  void SetLoopExitIVInfo(IVInfo* loop_exit_iv) {
 ...
 private:
...
#ifdef EXYNOS_ART_OPT
  bool loop_inversion_;
  HBasicBlock* old_back_edge_;
  ...
  GrowableArray<IVInfo*> iv_info_;
  GrowableArray<RVInfo*> rv_info_;
  IVInfo* biv_info_;
  bool is_count_up_loop_;

  GrowableArray<HBasicBlock*> exiting_blocks_;

  GrowableArray<ArrayAccessInfo*> array_access_info_;

  HCondition* exit_cond_;
  bool inclusive_;

  /* Induction variable used in loop exit condition test */
  IVInfo* loop_exit_iv_;
#endif
...
class HBasicBlock : public ArenaObject<kArenaAllocMisc> {
...
#ifdef EXYNOS_ART_OPT
  void ClearDominatedBlocks() { dominated_blocks_.Reset(); }
#endif
 ...
 private:
  ...
  friend class HInstruction;
#ifdef EXYNOS_ART_OPT
  friend class HGraphOpt;
  friend class LoopVectorization;
#endif
...
#ifdef EXYNOS_ART_OPT
#define FOR_EACH_PACKED_INSTRUCTION_COMMON(M)                           \
  M(PackedPhi, Phi)                                                     \
  M(PackedConst, Instruction)                                           \
  M(PackedMov, Instruction)                                             \
  M(PackedBinaryOperation, BinaryOperation)                             \
  M(PackedAdd, PackedBinaryOperation)                                   \
  M(PackedMul, PackedBinaryOperation)                                   \
  M(PackedInitIndVar, Instruction)                                      \
  M(PackedInitRdxVar, Instruction)                                      \
  M(VectorToScalar, Instruction)                                        \
  M(VectorToScalarForAddRdxOp, VectorToScalar)                          \

#define FOR_EACH_PACKED_INSTRUCTION_ARM64(M)                            \
  M(PackedPhiARM64, PackedPhi)                                          \
  M(PackedConstARM64, PackedConst)                                      \
  M(PackedMovARM64, PackedMov)                                          \
  M(PackedAddARM64, PackedAdd)                                          \
  M(PackedMulARM64, PackedMul)                                          \
  M(PackedInitIndVarARM64, PackedInitIndVar)                            \
  M(PackedInitRdxVarARM64, PackedInitRdxVar)                            \
  M(VectorToScalarForAddRdxOpARM64, VectorToScalarForAddRdxOp)          \

#define FOR_EACH_INSTRUCTION(M)                                         \
  FOR_EACH_CONCRETE_INSTRUCTION(M)                                      \
  FOR_EACH_PACKED_INSTRUCTION_COMMON(M)                                 \
  FOR_EACH_PACKED_INSTRUCTION_ARM64(M)                                  \
...
#ifdef EXYNOS_ART_OPT
class HPackedPhi : public HPhi {
 public:
  HPackedPhi(ArenaAllocator* arena, uint32_t reg_number, size_t number_of_inputs,
             Primitive::Type type)
    : HPhi(arena, reg_number, number_of_inputs, type) {}

  DECLARE_INSTRUCTION(PackedPhi);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedPhi);
};

class HPackedPhiARM64 : public HPackedPhi {
 public:
  HPackedPhiARM64(ArenaAllocator* arena, uint32_t reg_number, size_t number_of_inputs)
    : HPackedPhi(arena, reg_number, number_of_inputs, Primitive::kPrimFloat) {}

  DECLARE_INSTRUCTION(PackedPhiARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedPhiARM64);
};

class HPackedConst : public HExpression<0> {
 public:
  HPackedConst(Primitive::Type type, uint32_t element_size, int32_t imm32, uint64_t imm64)
    : HExpression(type, SideEffects::None()),
  ...
  DECLARE_INSTRUCTION(PackedConst);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedConst);
};

class HPackedConstARM64 : public HPackedConst {
 public:
  HPackedConstARM64(uint32_t element_size, int32_t imm32, uint64_t imm64,
                  VectorFormat vectorFormat)
  : HPackedConst(Primitive::kPrimFloat, element_size, imm32, imm64),
    vectorFormat_(vectorFormat) {}

  VectorFormat vectorFormat_;

  DECLARE_INSTRUCTION(PackedConstARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedConstARM64);
};

class HPackedMov : public HExpression<1> {
 public:
  HPackedMov(Primitive::Type type, HInstruction* input)
    : HExpression(type, SideEffects::None()) {
    SetRawInputAt(0, input);
  }
  ...
  DECLARE_INSTRUCTION(PackedMov);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedMov);
};

class HPackedMovARM64 : public HPackedMov {
 public:
  HPackedMovARM64(HInstruction* input, VectorFormat vectorFormat)
    : HPackedMov(Primitive::kPrimFloat, input),
      vectorFormat_(vectorFormat) {}

  VectorFormat vectorFormat_;

  DECLARE_INSTRUCTION(PackedMovARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedMovARM64);
};

class HPackedBinaryOperation : public HBinaryOperation {
 public:
  HPackedBinaryOperation(Primitive::Type result_type, HInstruction* left, HInstruction* right)
    : HBinaryOperation(result_type, left, right) {}
  ...
  DECLARE_INSTRUCTION(PackedBinaryOperation);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedBinaryOperation);
};

class HPackedAdd : public HPackedBinaryOperation {
 public:
  HPackedAdd(Primitive::Type result_type, HInstruction* left, HInstruction* right,
           bool isFloatingPointType)
    : HPackedBinaryOperation(result_type, left, right),
      isFloatingPointType_(isFloatingPointType) {}

  bool IsCommutative() const OVERRIDE { return true; }

  bool isFloatingPointType_;

  DECLARE_INSTRUCTION(PackedAdd);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedAdd);
};

class HPackedAddARM64 : public HPackedAdd {
 public:
  HPackedAddARM64(HInstruction* left, HInstruction* right, bool isFloatingPointType,
                VectorFormat vectorFormat)
    : HPackedAdd(Primitive::kPrimFloat, left, right, isFloatingPointType),
      vectorFormat_(vectorFormat) {}

  VectorFormat vectorFormat_;

  DECLARE_INSTRUCTION(PackedAddARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedAddARM64);
};

class HPackedMul : public HPackedBinaryOperation {
 public:
  HPackedMul(Primitive::Type result_type, HInstruction* left, HInstruction* right,
           bool isFloatingPointType)
    : HPackedBinaryOperation(result_type, left, right),
      isFloatingPointType_(isFloatingPointType) {}
  ...
  DECLARE_INSTRUCTION(PackedMul);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedMul);
};

class HPackedMulARM64 : public HPackedMul {
 public:
  HPackedMulARM64(HInstruction* left, HInstruction* right, bool isFloatingPointType,
                VectorFormat vectorFormat)
    : HPackedMul(Primitive::kPrimFloat, left, right, isFloatingPointType),
   vectorFormat_(vectorFormat) {}

  VectorFormat vectorFormat_;

  DECLARE_INSTRUCTION(PackedMulARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedMulARM64);
};

class HPackedInitIndVar : public HExpression<0> {
public:
  HPackedInitIndVar(Primitive::Type type, uint32_t loop_step, bool isFloatingPointType,
                    bool is64bits)
    : HExpression(type, SideEffects::None()),
  ...
  DECLARE_INSTRUCTION(PackedInitIndVar);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedInitIndVar);
};

class HPackedInitIndVarARM64 : public HPackedInitIndVar {
 public:
  HPackedInitIndVarARM64(uint32_t loop_step, bool isFloatingPointType, bool is64bits,
                       VectorFormat vectorFormat)
    : HPackedInitIndVar(Primitive::kPrimFloat, loop_step, isFloatingPointType, is64bits),
      vectorFormat_(vectorFormat) {}

  VectorFormat vectorFormat_;

  DECLARE_INSTRUCTION(PackedInitIndVarARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedInitIndVarARM64);
};

class HPackedInitRdxVar : public HExpression<1> {
public:
  HPackedInitRdxVar(Primitive::Type type, HInstruction* input, bool isFloatingPointType,
                  bool is64bits, uint32_t element_size, uint32_t init_val)
    : HExpression(type, SideEffects::None()),
      isFloatingPointType_(isFloatingPointType),
      is64bits_(is64bits),
      element_size_(element_size),
  ...
  DECLARE_INSTRUCTION(PackedInitRdxVar);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedInitRdxVar);
};

class HPackedInitRdxVarARM64 : public HPackedInitRdxVar {
 public:
  HPackedInitRdxVarARM64(HInstruction* input, bool isFloatingPointType,
                       bool is64bits, uint32_t element_size, uint32_t init_val,
                       VectorFormat vectorFormat)
    : HPackedInitRdxVar(Primitive::kPrimFloat, input, isFloatingPointType, is64bits, element_size,
                      init_val),
      vectorFormat_(vectorFormat) {}

  VectorFormat vectorFormat_;

  DECLARE_INSTRUCTION(PackedInitRdxVarARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HPackedInitRdxVarARM64);
};

class HVectorToScalar : public HExpression<1> {
 public:
  HVectorToScalar(Primitive::Type type, HInstruction* input, bool isFloatingPointType,
                  uint32_t element_size, bool is_quad_packed, bool is_type_cast_considered,
                  bool is_int_to_short)
    : HExpression(type, SideEffects::None()),
      isFloatingPointType_(isFloatingPointType),
      element_size_(element_size),
      is_quad_packed_(is_quad_packed),
      is_type_cast_considered_(is_type_cast_considered),
      is_int_to_short_(is_int_to_short) {
    SetRawInputAt(0, input);
  }

  bool isFloatingPointType_;
  uint32_t element_size_;
  bool is_quad_packed_;
  bool is_type_cast_considered_;
  bool is_int_to_short_;

  DECLARE_INSTRUCTION(VectorToScalar);

private:
  DISALLOW_COPY_AND_ASSIGN(HVectorToScalar);
};

class HVectorToScalarForAddRdxOp : public HVectorToScalar {
 public:
  HVectorToScalarForAddRdxOp(Primitive::Type type, HInstruction* input, bool isFloatingPointType,
                             uint32_t element_size, bool is_quad_packed,
                             bool is_type_cast_considered, bool is_int_to_short)
    :  HVectorToScalar(type, input, isFloatingPointType, element_size, is_quad_packed,
                       is_type_cast_considered, is_int_to_short) {}

  DECLARE_INSTRUCTION(VectorToScalarForAddRdxOp);

private:
  DISALLOW_COPY_AND_ASSIGN(HVectorToScalarForAddRdxOp);
};

class HVectorToScalarForAddRdxOpARM64 : public HVectorToScalarForAddRdxOp {
 public:
  HVectorToScalarForAddRdxOpARM64(Primitive::Type type, HInstruction* input,
                                  bool isFloatingPointType, uint32_t element_size,
                                  bool is_quad_packed, bool is_type_cast_considered,
                                  bool is_int_to_short, VectorFormat vectorFormat)
    :  HVectorToScalarForAddRdxOp(type, input, isFloatingPointType, element_size,
                                  is_quad_packed, is_type_cast_considered, is_int_to_short),
       vectorFormat_(vectorFormat) {}

  VectorFormat vectorFormat_;

  DECLARE_INSTRUCTION(VectorToScalarForAddRdxOpARM64);

 private:
  DISALLOW_COPY_AND_ASSIGN(HVectorToScalarForAddRdxOpARM64);
};
#endif
