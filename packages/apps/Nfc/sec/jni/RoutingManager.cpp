...
/* END [J14121701] - aid matching value from partial aid configuration */
    ...
    memset (&mSeTechMaskonSubScreenState, 0x03, sizeof(mSeTechMaskonSubScreenState));
    memset (&mSeProtoMaskonSubScreenState, 0x08, sizeof(mSeProtoMaskonSubScreenState));
...
/* START [160607001J] - Setup NFCEE configuration after enabling NFCEE */
...
/* START [160616002P] - Update LMRT if it update by req notification.*/
    ...
    tNFA_PROTOCOL_MASK nSeProtoMask = 0x00;
...
/* END [P160421001] - Patch for Dynamic SE Selection */
        {
            ...
            bIsUpdateLMRT = true;
/* START [160616002P] - Update LMRT if it update by req notification.*/
            // Clear listen tech mask for previous DefaultEe
            mSeTechMask = 0x00;
            mSeTechMaskonSubScreenState[0] = 0x00;
            mSeTechMaskonSubScreenState[1] = 0x00;
            mSeTechMaskonSubScreenState[2] = 0x00;
/* END [160616002P] - Update LMRT if it update by req notification.*/

            if (mEeInfo.ee_disc_info[i].la_protocol != 0)
            {
                mSeTechMask |= NFA_TECHNOLOGY_MASK_A;
                mSeTechMaskonSubScreenState[0] |= NFA_TECHNOLOGY_MASK_A;
                mSeTechMaskonSubScreenState[1] |= NFA_TECHNOLOGY_MASK_A;
                mSeTechMaskonSubScreenState[2] |= NFA_TECHNOLOGY_MASK_A;
            }
            /* START [J15012601] - Listen Tech Mask for CE & HCE */
            if (mEeInfo.ee_disc_info[i].lb_protocol != 0)
            {
                mSeTechMask |= NFA_TECHNOLOGY_MASK_B;
                mSeTechMaskonSubScreenState[0] |= NFA_TECHNOLOGY_MASK_B;
                mSeTechMaskonSubScreenState[1] |= NFA_TECHNOLOGY_MASK_B;
                mSeTechMaskonSubScreenState[2] |= NFA_TECHNOLOGY_MASK_B;
            }
            /* END [J15012601] - Listen Tech Mask for CE & HCE */
...
/* START [160616001P] - Added CE listen tech for F-Type UICC*/
            if (mEeInfo.ee_disc_info[i].lf_protocol != 0)
            {
                ...
                mSeTechMaskonSubScreenState[2] |= NFA_TECHNOLOGY_MASK_F;
            }
...
/* START [P1605170002] - Change CE listen tech configuration method for support 0x00 tech*/
            ...
            nfaStat = NFA_EeSetDefaultTechRouting(mEeInfo.ee_disc_info[i].ee_handle, mSeTechMask, mSeTechMask,
                    mSeTechMask, mSeTechMaskonSubScreenState);
            if (nfaStat != NFA_STATUS_OK)
...
/* END [P1605170002] - Change CE listen tech configuration method for support 0x00 tech*/
            {
                ...
                nSeProtoMask |= NFA_PROTOCOL_MASK_ISO_DEP;
            }
...
/* START [160616001P] - Added CE listen tech for F-Type UICC*/
            ...
            {
                // Set protocol routes to DefaultSE if it's thers
                nSeProtoMask |= NFA_PROTOCOL_MASK_T3T;
            }
/* END [160616001P] - Added CE listen tech for F-Type UICC*/

            if(nSeProtoMask != 0x00)
            {
                nfaStat = NFA_EeSetDefaultProtoRouting(mDefaultEe, nSeProtoMask,
                        nSeProtoMask, nSeProtoMask, mSeProtoMaskonSubScreenState);
                if (nfaStat != NFA_STATUS_OK)
...
/* END [160616002P] - Update LMRT if it update by req notification.*/
    ...
    ALOGD ("%s ....mSeTechMask(0x%x)......... Exit.", fn, mSeTechMask);
}
/* END [160607001J] - Setup NFCEE configuration after enabling NFCEE */
...
/* START [160607001J] - Setup NFCEE configuration after enabling NFCEE */
        ...
        nfaStat = NFA_EeSetDefaultTechRouting (mDefaultEe, mSeTechMask, mSeTechMask, mSeTechMask, mSeTechMaskonSubScreenState);
        if (nfaStat == NFA_STATUS_OK)
            mRoutingEvent.wait ();
        else
...
/* END [160607001J] - Setup NFCEE configuration after enabling NFCEE */
        ...
        if (mDefaultEe == 0x00)
        {
            ...
            nfaStat = NFA_EeSetDefaultProtoRouting(mDefaultEe, NFA_PROTOCOL_MASK_ISO_DEP, 0, 0, mSeProtoMaskonSubScreenState);
            if (nfaStat == NFA_STATUS_OK)
                mRoutingEvent.wait ();
            else
...
/* START [160616001P] - Added CE listen tech for F-Type UICC*/
            ...
            ALOGD("%s: [2] mDefaultEe(0x%x) , Protoocl(0x%x),  ", __FUNCTION__, mDefaultEe, NFA_PROTOCOL_MASK_T3T);
            if(nSeProtoMask != 0x00)
            {
                nfaStat = NFA_EeSetDefaultProtoRouting(mDefaultEe, nSeProtoMask,
                    nSeProtoMask, nSeProtoMask, mSeProtoMaskonSubScreenState);
...
/* START [J14127007] - Setup NFCEE configuration after enabling NFCEE */
            ...
            {
                tNFA_STATUS nfaStat;
                nfaStat = NFA_CeConfigureUiccListenTech (routingManager.mDefaultEe, mUiccListenMask);
...
void RoutingManager::setCeDiscoveryMask (unsigned long tech_mask)
{
    ...
    ALOGD("%s: Enter - defaultEe = 0x%02X, mUiccListenMask = 0x%02X", __FUNCTION__, gCurrentDefaultEe, mUiccListenMask);

    useAidOnDH(false);

/* START [P1605170002] - Change CE listen tech configuration method for support 0x00 tech*/
    NFA_CeSetIsoDepListenTech(mUiccListenMask);
    NFA_CeConfigureUiccListenTech (gCurrentDefaultEe, mUiccListenMask);
/* END [P1605170002] - Change CE listen tech configuration method for support 0x00 tech*/
...
void RoutingManager::setCeListenTech (unsigned long tech_mask)
{
    ...
    for (UINT8 i = 0; i < mEeInfo.num_ee; i++)
    {
        ...
        NFA_CeConfigureUiccListenTech (seId, 0);
        if (gCurrentDefaultEe == seId)
            isDefaultEe = true;
    }
...
{
    UINT16 tech = (UINT16)value & mUiccListenMask ;
...
/* START [J14120901] - set ce listen tech */
        if (proto & NFA_PROTOCOL_MASK_ISO_DEP)
        {
            gCurrentDefaultEe = seId;

/* START [P160421001] - Patch for Dynamic SE Selection */
            mDefaultEe = seId;
            ALOGD("%s: - mDefaultEe(0x%x), mOffHostEe(0x%x)", __FUNCTION__, mDefaultEe, mOffHostEe);
/* END [P160421001] - Patch for Dynamic SE Selection */
        }
/* END [J14120901] */
    }
