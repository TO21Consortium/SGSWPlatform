...
/* START [160503001J] Configure HCI setting for wired C/E by eSE chip type (Oberthure or Gemalto)*/
    ...
    if (GetNumValue("NFA_HCI_DEFAULT_STATIC_PIPE_TYPE", &num, sizeof(num)))
    {
        if(num == 0x00)
        {
            ...
            if (GetNumValue("NFA_HCI_APDU_GATE", &num, sizeof(num)))
                gGatePipe = num;
            else
                gGatePipe = 0xF0;       // 0xF0 : APDU gate for GP.
        }
    }
    ALOGD ("%s: Default Pipe Type (1 : Static, 0 : Dynamic) = 0x%X", fn, gUseStaticPipe);

    if (GetNumValue("NFA_HCI_SUPPORT_CONCURRENT_WCE", &num, sizeof(num)))
        gIsSupportConcurrentWCE = num;
...
bool SecureElement::connectEE ()
{
    ...
    ALOGD ("%s: enter, mActiveEeHandle: 0x%04x, SEID: 0x%x, pipe_gate_num=%d, gUseStaticPipe=%d",
