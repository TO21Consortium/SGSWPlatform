...
/* START [16052901J] - Change listen tech mask values */
    jboolean nfcManager_changeListenTechMask(JNIEnv* e, jobject o, jint listen_techmask);
/* END [16052901J] - Change listen tech mask values */
...
static SyncEvent            sNfaChangeListenTechEvent;  //event for NFA_ChangeListening(
...
//#define SEID_GEN_ESE    0xF4
//#define SEID_GEN_UICC   0xF3
...
/* START [16052901J] - Change listen tech mask values */
    case NFA_LISTEN_CHANGED_EVT:
        ALOGD ("%s: NFA_LISTEN_CHANGED_EVT", __FUNCTION__);
        SyncEventGuard guard (sNfaChangeListenTechEvent);
        sNfaChangeListenTechEvent.notifyOne ();
        break;
/* END [16052901J] - Change listen tech mask values */
    }
}
...
/*******************************************************************************
**
** Function:        nfcManager_changeListenTechMask
**
...
jboolean nfcManager_changeListenTechMask(JNIEnv* e, jobject o, jint listen_techmask)
{
    ...
    ALOGD ("%s: enter  listen_techmask = 0x%02X", __FUNCTION__, listen_techmask);

    startRfDiscovery(false);

    {
        SyncEventGuard guard (sNfaChangeListenTechEvent);
        status = NFA_ChangeListening(listen_techmask);
        if (status == NFA_STATUS_OK)
        {
            if (sNfaChangeListenTechEvent.wait (500) == false)
                ALOGE ("%s: NFA_ChangeListening() function timed out", __FUNCTION__);
        }
        else
            ALOGE ("%s: fail changing listen tech operation.", __FUNCTION__);
    }

    startRfDiscovery(true);

    ALOGD ("exit", __FUNCTION__);
    return JNI_TRUE;
}
/* END [16052901J] - Change listen tech mask values */
