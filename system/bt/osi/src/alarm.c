...
static void reschedule_root_alarm(void) {
  ...
  if (next_expiration < TIMER_INTERVAL_FOR_WAKELOCK_IN_MS) {
    ...
    if (timer_settime(wakeup_timer, TIMER_ABSTIME, &wakeup_time, NULL) == -1)
      LOG_ERROR("%s unable to set wakeup timer: %s",
...
static bool release_wake_lock(void) {
  ...
  if (wrote_name_len == -1) {
    LOG_ERROR("%s can't release wake lock: %s",
              __func__, strerror(errno));
  } else if (wrote_name_len < locked_id_len) {
    LOG_ERROR("%s lock release only wrote %zd, assuming released",
...
static bool timer_create_internal(const clockid_t clock_id, timer_t *timer) {
  ...
  if (timer_create(clock_id, &sigevent, timer) == -1) {
    LOG_ERROR("%s unable to create timer with clock %d: %s",
